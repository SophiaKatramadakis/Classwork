...
void updateLEDs(int bpm, int spo2) {
 // Debugging: Print bpm and spo2 values
 Serial.print("Updating LEDs with BPM: ");
 Serial.print(bpm);
 Serial.print(" and SpO2: ");
 Serial.println(spo2);


 digitalWrite(GREEN_LED, LOW); // Turn off all LEDs initially
 digitalWrite(BLUE_LED, LOW);
 digitalWrite(RED_LED, LOW);


 if (bpm >= 50 && bpm <= 100 && spo2 > 94) {
   digitalWrite(GREEN_LED, HIGH); // Healthy levels
   Serial.println("Green LED ON");
 } else if (((bpm < 50 && bpm > 40) || bpm > 100) || (spo2 >= 90 && spo2 <= 94)) {
   digitalWrite(RED_LED, HIGH); // Caution levels
   digitalWrite(GREEN_LED, HIGH);
   Serial.println("Yellow LED ON");
 } else {
   digitalWrite(RED_LED, HIGH); // Critical levels
